...
Procedure Process(client) 
  Protected length,*buffer,*mem,res,pos,input.s,prompt.s,content.s,title.s
  ...
  *buffer = AllocateMemory($FFFF) 
  ...
  If PeekS(*buffer,3,#PB_UTF8) = "GET"  
    ...
    If pos 
      ...
      lchat() = "<p> Question: " + input + "</p>" + "<p> Answer: " + gResponce + "</p>" +  #CRLF$   
      
    EndIf 
    
    title. s = "PB Gpt4all Chat"  
    
    content.s = "<!DOCTYPE html>" + #CRLF$
    content + "<html><head>" + #CRLF$
    content + "<meta charset='utf-8' />" + #CRLF$
    content + "<title>" + title + "</title>" + #CRLF$
    ...
    content + "</head><body> <h1>" + title + "</h1>"+ #CRLF$
...
Global Event,ServerEvent,ClientID,gbusy,prompt.s 
